...
[A. 코치 능동적 보강 등록 — 권장 기본 흐름]
  ...
  코치 7-0 화면: 상태처리하기 → 보강 처리 클릭 시 2단계 바텀시트:

  [1단계] 보강 사유 바텀시트 (makeup-reason-sheet):
    - 사유 선택 (필수): 고객 사정 / 코치 사정
    - 상세 입력 (선택): 자유 텍스트
    - 사유 선택 시 "다음: 날짜 선택하기" 버튼 활성화
    - 회차 차감 없음 안내 표시

  [2단계] 보강 날짜 선택 바텀시트 (makeup-date-sheet):
    - 7일 단위 주 이동 (과거 주 이동 불가)
    - 날짜 클릭 → 해당 요일 가능 시간 슬롯 표시 (코치 MAKEUP_SLOTS_BY_DAY 기준)
    - 복수 날짜/시간 선택 가능
    - 선택 수 뱃지 표시 (날짜 셀에 선택된 슬롯 수 표시)
    - "선택 날짜 제안하기 (N개)" → 수강생에게 카카오 알림톡 발송
    - 슬롯 선택 없을 시 버튼 비활성

  - status: (레슨예정/결강) → MAKEUP_PENDING
...
통합 대상 분류 (2종):
  A. 결강(ABSENT) 회차 — 수강생에게 결강 처리된 회차 (정책상 보강 가능 유형만)
  B. 예정(CONFIRMED) 회차 — 수강생의 같은 코치 예정 레슨 중 미래 일정

  케이스 1 — "결강 + 결강" 통합:
    결강된 회차 N건을 새로운 보강 슬롯 1개로 묶기
    → 코치가 새 일정을 선택해야 함
    예: 4/13 결강(20분) + 4/20 결강(20분) → 새 4/27 (일) 10:00 (40분)

  케이스 2 — "결강 + 예정" 통합:
    결강된 회차 1건을 같은 수강생의 기존 예정 회차에 흡수시켜 길게 진행
    → 일정은 예정 회차 그대로, 그 회차의 시간만 늘림
    예: 4/29 결강(20분) + 5/2 (금) 예정(20분) → 5/2 (금)에 40분으로 길게 진행

  케이스 3 — "예정 + 예정" 통합:
    같은 수강생의 인접한 예정 회차 2건을 한 번에 묶기 (둘 다 결강 아님)
    → 합쳐질 일정 1개를 코치가 선택, 다른 회차는 흡수
    1차 릴리스 범위 외 (수요 검증 후 추가)

플로우 — 통합 (MERGE):
  [전제] 통합 대상이 본인 회차 외 1건 이상 존재 (결강 또는 예정)
  [1] 코치가 7-0 → 결강 카드 → "보강 처리" 클릭
  [2] 보강 사유 바텀시트에서 사유 선택 후, 신규 토글 "회차 통합/분할 제안"을 ON
  [3] 라디오: "통합" / "분할" — 통합 선택
  [4] 통합 대상 회차 체크박스 리스트 — 두 종류 동시 노출 (라벨로 구분):
    · [결강] 다른 결강 회차 (사유 표기)
    · [예정] 가까운 예정 회차 (D+N 표기, 같은 코치-수강생-1:1 매칭)
  [5] 합산 시간 미리보기 — 케이스에 따라 분기:
    · 케이스 1: "20분 × 2회 → 40분 × 1회 (코치가 일정 선택 필요)"
    · 케이스 2: "20분(결강) + 20분(5/2 금 예정) → 5/2 (금) 10:00 · 40분으로 길게 진행"
  [6] "다음" 처리 분기:
    · 케이스 1 → 보강 날짜 바텀시트 (슬롯 1개 선택)
    · 케이스 2 → 날짜 선택 단계 스킵, 곧바로 제안 확정 모달 (안내: "기존 5/2 금요일 일정에 합쳐서 진행됩니다")
  [7] 제안하기 → 수강생 알림톡 (MAKEUP_MERGE_PROPOSED, 케이스 변수로 본문 분기)
  [8] 수강생 수락 시:
    · 케이스 1:
      - 신규 Booking 생성 (lessonDuration=MIN_40, sessionAdjustmentType=MERGE, sessionWeight=2, mergeGroupId=UUID)
      - 원 회차 N건의 mergedIntoBookingId = 신규 Booking.id, status=ABSENT 유지
    · 케이스 2:
      - 기존 예정(CONFIRMED) Booking의 lessonDuration을 MIN_40으로 갱신, sessionAdjustmentType=MERGE, sessionWeight=2, mergeGroupId=UUID 채움
      - 결강 회차 1건의 mergedIntoBookingId = 예정 Booking.id, status=ABSENT 유지
      - **슬롯 점유 재계산** — 예정 회차의 시작 시각 + 추가 20분(또는 +N분)에 다른 예약/홀드가 있으면 거부 (수락 시점에 충돌 검사)
    · 결과: 새/기존 통합 Booking 1건이 회차 카운트에 sessionWeight(=2)만큼 기여
...
수강생 측 UI 흐름 (6-6 신청 상세):
  - 알림톡 도착 → "보러가기" 딥링크 → 6-6 (status=merge_proposed / split_proposed)
  - 보라 테마 박스 + 변경 미리보기(원 회차 N건 취소선 → 통합/분할 후 회차 강조)
  - 영향 안내 (회차 카운트 변동 + 결제 변동 없음 + 결강 시 정책)
  - 하단 [거절] / [통합 수락 | 분할 수락] 버튼 (보라색 강조)
  - 거절 시 confirm() → 원 상태 복귀 (통합 거절 → MAKEUP_REQUESTED, 분할 거절 → MAKEUP_PENDING)
  - 수락 시 → 6-6 (status=merge_confirmed / split_confirmed) 화면으로 이동

확정 후 양측 표시 (7-0 코치 / 6-6 수강생 공통):
  - 배지: "🔗 통합 회차" / "✂ 분할 회차" (보라 #5B21B6)
  - 회차 라벨: 통합 = "N/M회 (통합 +K)", 분할 = "N/M회 (분할 1/K)"
  - 레슨 시간 라벨: 통합 = "40분 (통합)", 분할 = "20분 (분할 1/2)"
  - 그룹 박스: 통합 = 원 회차 N건 일정 리스트(취소선) / 분할 = 원 회차(취소선) + 같은 그룹 회차 리스트(현재 강조)
```
