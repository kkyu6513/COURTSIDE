...
레슨 상세 풀팝업:
  ...
  결제 상태 정책:
    ...
    - 외부 결제: isPaid=true, paymentId=null (코치가 수동 확인). 파란색 "외부결제" 표시
      코치가 레슨 상세에서 "외부 결제 확인" 버튼으로 수동 전환 가능
  ...
  - 상태별 하단 액션:
    [진행중]   레슨 완료 처리 (단일 버튼)
    [레슨 예정] "상태 처리하기" 버튼 1개 → 바텀시트:
      🏁 레슨 완료 — 정상 완료 처리
      ➕ 보강 처리 — 추가 보강 레슨 등록 (bookingType=SUPPLEMENTARY)
      ✕ 결강 처리 — 확인 다이얼로그 → 결강 처리 + 수강생 알림
    [변경 요청] 모두 제안하기 또는 불가 알림 보내기
    [대기 중]   더보기(보류/거절/고객과 직접 일정잡기) + 스케줄 확정하기
    [결강]     결강 사유 표시 (고객/코치 사유 + 상세). 닫기 버튼만
    [보강대기]  원래 결강 일정(취소선) + 제안된 보강 일정 + 홀드 남은 시간. 닫기 버튼
    [보강확정]  원래 결강 일정(취소선) + 확정된 보강 일정. 닫기 버튼
    [레슨완료]  닫기
