...
레슨 상세 풀팝업:
  ...
  - [변경 요청] 상태: 레슨 정보(유형/형태/결제) 하단에 "변경 요청" 타이틀로 영역 구분 후
    변경 요청 UI 배치: 기존 일정(취소선) + 수강생 희망 날짜 + 자동 추천 가능 날짜(칩, 다중 선택 가능)
    수강생 희망 날짜가 코치 가용 시간에 포함되면 희망 날짜 우측에 [확정하기] 버튼 표시
...
```
보강처리: 레슨이 정상 진행되지 못했을 때 보강 레슨을 등록하는 기능

보강처리 플로우:
  1. 레슨 상세에서 "보강처리" 버튼 클릭
  2. 사유 선택 (라디오 버튼, 필수):
     - 고객 사유: 수강생의 사정으로 레슨 불참/취소 (예: 개인 사정, 건강, 일정 충돌)
     - 코치 사유: 코치의 사정으로 레슨 진행 불가 (예: 건강, 긴급 일정, 코트 문제)
  3. 상세 사유 입력 (텍스트, 필수)
  4. 저장 → PUT /api/bookings/:id/makeup { reasonType: 'STUDENT' | 'COACH', reason: '...' }
     → Booking.status = MAKEUP_PENDING (보강 대기)
     → 보강 레슨 자동 생성 (미확정 상태)
     → 수강생에게 보강 안내 알림톡 발송

보강 레슨 정책:
  - 고객 사유: 해당 회차는 소진으로 처리, 보강 레슨 1회 추가 제공 (코치 재량)
  - 코치 사유: 해당 회차 미소진, 보강 레슨 1회 필수 제공
  - 보강 레슨은 원래 레슨과 동일한 조건 (형태, 시간)으로 생성
  - 보강 레슨 일정은 코치-수강생 간 협의 후 확정
```
