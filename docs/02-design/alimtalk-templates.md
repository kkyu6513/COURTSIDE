...
#### STUDENT_SELF_CLAIM — 매칭 성공한 코치에게 발송
...
**발송 트리거**: `submitStudentProfile` Server Action에서 학생 입력 코치 번호로 `users` 조회 → `role=COACH AND phone=입력번호` 매칭 시
**설정 항목**: 항상 (사용자가 끌 수 없음 — 핵심 운영 알림)
**변수**:
| 변수 | 출처 | 예시 |
|------|------|------|
| #{학생명} | user.user_metadata.nickname (카카오) 또는 user.email | 홍길동 |

**API 호출**: `submitStudentProfile` 내부 `sendSms()` (fire-and-forget)
**결과 기록**: `student_self_claims.notifiedAt / notifyAttempts / notifyLastError`
**미발송 케이스**:
- `SKIPPED: ENV_NOT_SET` — Solapi 환경변수(SOLAPI_API_KEY, SOLAPI_API_SECRET, SOLAPI_FROM) 미설정
