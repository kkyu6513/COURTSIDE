...
| FR-03a | 홈 화면 | **하단 네비게이션 바**: 홈 / 메시지 / 검색 / 마이메뉴 (4탭 고정) | High | Pending |
...
| FR-12b-0 | 마이페이지 | **코치 스케줄 홈 (코치 전용 첫 화면)**: 코치 로그인 시 **스플래시 → 스케줄 홈 화면으로 이동** (수강생/비로그인은 홈). 구성: 주간 미니 캘린더 + 오늘 레슨 일정 (확정/대기 상태별 색상) + 대기 중 신청 목록 + 이번 주 요약 (총 레슨/대기/완료). **코치 전용 하단 네비게이션**: 📅 스케줄(홈) / 💬 메시지 / 🔍 검색 / 👤 마이 (수강생과 1번 탭만 다름) | High | Pending |
