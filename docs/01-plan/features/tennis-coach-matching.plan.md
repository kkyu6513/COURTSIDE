...
> **Version**: 2.0.0 (Coach SaaS 피벗)
> **Author**: User
> **Date**: 2026-04-09 (v1.0.0) / 2026-05-08 (v2.0.0 피벗)
> **Status**: Draft

---

## ⚠️ Phase 2 — Coach SaaS 피벗 (2026-05-08)

> 이 섹션이 v1.0의 매칭 마켓플레이스 비전보다 **우선** 적용된다. 아래 v1.0 섹션(Executive Summary, FR 표 등)은 히스토리/참고용으로 보존하되, ⛔ DEPRECATED 마커가 붙은 항목은 현 단계 스펙·구현 대상에서 제외한다.

### 피벗 배경

매칭 마켓플레이스로 시작했으나 코치 운영 도구(스케줄·보강·회차 통합·결제 정책 등)가 압도적으로 깊어진 상태. 한국 매칭 시장은 숨고/탈잉이 점유 중이라 차별화가 어렵고, 코치 전용 SaaS는 빈 시장이라 판단해 **공급자(코치) SaaS 우선 → 코치 풀 형성 후 매칭 가능** 순서로 전환.

### 새 비즈니스 정의

| Key | Value |
|---|---|
| **Primary user** | 테니스 코치 (구독 결제 주체, SaaS 핵심 고객) |
| **Secondary user** | 코치가 등록·초대한 학생 (가입 시 자동으로 그 코치에 연결, 마켓플레이스 탐색 X) |
| **Core value** | 코치가 학생을 체계적으로 관리 (스케줄·보강·결제·회차·변경 요청 일원화) |
| **Revenue** | 코치 월 구독료 (단순 SaaS 모델, 거래 수수료 없음) |
| **Acquisition** | 코치를 손으로 끌어다 쓰게 만들기 (콜드 아웃리치·콘텐츠·소개) |

### 보강 3종 옵션 (확정)

- **추가** — 기존 회차 차감 없이 새 날짜로 보강 (가장 일반적)
- **통합 (FR-14m)** — 짧은 회차 N개 → 긴 1회 (예: 결강 + 다음 정기 레슨에 합치기)
- **분할 (FR-14m)** — 긴 1회 → 짧은 N회

### FR 변동 요약

**⛔ DEPRECATED (마켓플레이스 모드 한정 — 현 단계 미구현)**:
- FR-03 홈(코치 노출), FR-03b 코치 목록, FR-04 매칭 추천
- FR-05 코치 상세 프로필, FR-05d 리뷰 목록 (학생 측 코치 탐색용)
- FR-05b 학생 → 코치 레슨 신청, FR-05c 코치 레슨신청 수락/거절
- FR-07a 레슨 예약 결제, FR-07b·c 환불 정책 (학생 결제는 코치 외부 정산으로 대체)
- FR-12a-3 찜한 코치
- FR-13 코치 검증 (관리자 매칭 신뢰성 강화 — SaaS엔 불필요)

**✏️ REDEFINED (의미 재정의)**:
- **FR-12b-2** 외부 수강생 등록 → **코치의 학생 등록·초대** 핵심 기능으로 격상 (전화번호 입력 → SMS/카카오 초대 링크 → 학생 가입 시 자동 연결)
- **FR-05c** (코치 신청 수신함) → **운영 알림함** (변경 요청 응답·보강 응답·결제 확인 통합)

**🆕 NEW (신규 필요)**:
- **FR-15a 학생 홈 = 내 스케줄** — 마켓플레이스 홈(FR-03) 대체. 학생이 앱 진입 시 보는 첫 화면을 코치 탐색이 아닌 "내 다가오는 레슨 캘린더"로 전환
- **FR-15b 코치 → 학생 초대** — 코치 화면에서 학생을 전화번호로 초대 (SMS/카카오 알림톡 링크 발송 → 링크 진입 시 자동 가입 + 그 코치에 연결)
- **FR-15c 학생 초대 가입 플로우** — 초대 링크 진입 시 가입 단계 단축 (역할 선택·코치 탐색 안내 화면 모두 스킵)

**그대로 유지**: FR-08 리뷰 (SaaS에서도 학생이 코치 평가 가능), 코치 운영 기능 전체 (FR-10, 10a, 11, 12b, 12b-1·3·4, 14m), 코치 가입·구독·프로필 (FR-01·02·06·07), 채팅(FR-05a), 알림톡(FR-09), 어드민 (FR-14 *).

### 화면 변동 요약

- **자름**: Flow 2 (홈/탐색) 전체 [2-1, 2-2, 2-3] / Flow 3 (레슨 신청) 전체 [3-1, 3-4, 3-5] / 6-7 찜한 코치 / 9-3 코치 검증
- **재정의**: 7-2 / 7-3 → 매칭 신청 수신함이 아닌 **운영 알림함** (변경 요청 응답·보강 응답 등)
- **신규**: 학생 신규 홈 (스케줄 캘린더), 코치 → 학생 초대 화면, 학생 초대 가입 플로우
...
| ⛔ FR-03b | 코치 탐색 | **(DEPRECATED — 마켓플레이스 모드)** 코치 목록 페이지: 지역·성별·수준·가격 필터, 평점 정렬 | High | Pending |
| ⛔ FR-04 | 코치 탐색 | **(DEPRECATED — 마켓플레이스 모드)** 매칭 추천: 학생 프로필 기반 코치 Top-5 추천 | High | Pending |
| ⛔ FR-05 | 코치 프로필 | **(DEPRECATED — 학생용 코치 탐색 화면. SaaS 피벗 후 학생은 자기 코치만 알면 됨)** **코치 프로필 상세 페이지**: 하단에 **문의하기** + **레슨신청** 버튼 구성. 우측 상단 **찜 버튼** (♡↔♥ 토글, 비로그인 시 로그인 유도). **PRO/ANNUAL 구독 배지는 관리자(ADMIN) 계정에서만 노출** (일반 사용자 미표시). **별점+리뷰 개수 클릭 → 리뷰 목록 화면(FR-05d)으로 이동**. 코치 상세 하단 리뷰 섹션 삭제 (리뷰는 별도 화면에서 조회) | High | Pending |
| ⛔ FR-05d | 코치 프로필 | **(DEPRECATED — 학생이 다른 코치 탐색하는 용도. SaaS는 학생-코치 1:1 관계라 불필요)** **코치 리뷰 목록 화면**: 코치 상세 → 별점/리뷰 개수 클릭 시 진입; 상단 평점 요약 (평균 점수 + 별점 분포 바 차트 1~5점) + 리뷰 카드 목록 (프로필 사진 + 작성자명 + 날짜 + 별점 + 리뷰 텍스트); API: `GET /api/coaches/:id/reviews` [공개] | High | Pending |
| FR-05a | 메시지 | **문의하기**: 코치-학생 간 1:1 DM 채팅 (Socket.io 기반 실시간 텍스트 채팅). **우측 상단 ⋯ 더보기 메뉴**: 채팅방 나가기 / 차단하기 / 신고하기. **📅 채팅 내 일정 제안**: 코치가 가용 슬롯 선택 → 카드형 메시지로 제안 → 수강생이 라디오로 1개 선택 → 일정 확정. 레슨 신청/변경 요청 모두 사용 가능 (추후 상세 디벨롭) | High | Pending |
| ⛔ FR-05b | 레슨 신청 | **(DEPRECATED — 학생→코치 신청 플로우. SaaS에선 코치가 학생을 등록·초대 — FR-15b로 대체)** **레슨신청 플로우 (2단계)**: ① 레슨 선택 (유형 + 세부항목 + 요일 + **코치가 등록한 가능 시간대만 노출** — `GET /api/coaches/:id/schedule`) → ② 확인·제출 — **신청 방식 선택**: "레슨 신청만" (미결제, 코치 수락 대기) 또는 **"결제 후 신청"** (토스페이먼츠 결제 → 바로 확정 + 서비스 내 스케줄 관리 기능 이용 가능)<br>**정규레슨**: 주 횟수(주 1·2·3회), 레슨 형태(1:1 개인 / 그룹 2~4인), 요일 다중 선택, 시간 1시간 단위 다중 선택<br>**쿠폰레슨**: 총 횟수(1~10회 선택), **유효기간(코치가 설정한 값 안내 문구로 노출)**, 레슨 형태(1:1 개인 / 그룹 2~4인), 요일 다중 선택 + **"선택한 요일은 희망 요일이며, 코치 확인 후 최종 확정됩니다" 안내 문구**, 시간 10분 단위 다중 선택 (코치 가능 시간만) | High | Pending |
| ✏️ FR-05c | 운영 알림함 | **(REDEFINED — 매칭 신청 수신함이 아닌 운영 알림함으로 재정의)** **코치 운영 알림함**: 변경 요청 응답 / 보강 응답 / 결제 확인 등 코치-학생 사이 운영 액션 통합 인박스. 카드 종류: 변경요청·보강제안·결제확인. 카드 클릭 → 해당 레슨 상세 풀팝업. 마켓플레이스 모드의 신청 수락/거절/취소는 ⛔ DEPRECATED. | High | Pending |
...
| ⛔ FR-07a | 레슨 결제 | **(DEPRECATED — 학생 측 레슨 결제는 코치 외부 정산으로 대체. 코치 구독 결제 FR-07만 유지)** 레슨 예약 결제: 날짜·시간 선택 → 토스페이먼츠 결제 → 결제 완료 콜백 → 예약 확정 | High | Pending |
| ⛔ FR-07b | 레슨 환불 | **(DEPRECATED — 학생 측 결제 자체가 SaaS에선 외부 처리)** **결제취소 및 환불정책 화면**: 레슨 신청 확인 화면(3-4)에서 "결제취소 및 환불정책 보기" 링크 → 바텀시트로 환불 정책 본문 표시. **데이터: Terms 테이블 (code=REFUND_POLICY)에서 조회** — 어드민 약관 관리(FR-14g)에서 등록·개정 가능. `GET /api/terms/REFUND_POLICY` [공개 API]. |
| ⛔ FR-07c | 레슨 환불 | **(DEPRECATED — SaaS 모드에선 학생 측 환불 불필요)** **환불 정책**: ① 정규레슨 — 3일 전 전액(100%), 1~2일 전 90%(위약금 10%), 당일 환불 불가 ② 쿠폰레슨 — 미사용 횟수 비례 환불 (사용 횟수 × 1회 금액 차감), 유효기간 만료 후 환불 불가 ③ 코치 책임 취소 — 100% 전액 환불 (노쇼, 코치 사유). 수강생 마이페이지 → 신청 내역 → 취소 버튼 → 환불 금액 안내 + 확인 → `POST /api/payments/:id/refund` (토스페이먼츠 환불 API) | High | Pending |
...
| ⛔ FR-12a-3 | 마이페이지 | **(DEPRECATED — 학생이 다른 코치 탐색·즐겨찾기하는 마켓플레이스 기능. SaaS는 1:1 코치-학생 관계라 불필요)** **찜한 코치**: 코치 카드 목록 (사진·이름·위치·전문분야·평점·구독 플랜 뱃지) + 레슨 신청 버튼 + 찜 해제 버튼; 정렬: 최근 찜한 순; 빈 상태 메시지 + 코치 탐색하기 버튼; 탈퇴·비활성 코치는 "활동 중지" 표시 후 하단 이동 | High | Pending |
...
| FR-12b-1 | 스케줄 | **전체 스케줄 보기**: 코치 스케줄 홈에서 "전체 스케줄 보기" 버튼 → 주간 타임테이블 풀팝업 (월~일 x 09:00~20:00, 상태별 색상). 셀 클릭 시 상세 토스트. **빈 슬롯 클릭 시**: ① 레슨 잡기 (신규 수강생 등록 — 이름/연락처/유형/형태 입력, 기존 수강생 선택 — 날짜 변경 또는 보강) ② 블록하기 (개인 일정 차단) | High | Pending |
| FR-12b-2 | 수강생관리 | **외부 수강생 등록 (코치 전용)**: 서비스 외부(오프라인) 수강생을 코치가 직접 등록. ExternalStudent 테이블 관리. 서비스 가입 시 전화번호 기준 자동 매칭. 회차 카운트 포함. 결제는 외부 처리 (paymentStatus='external') | High | Pending |
| FR-12b-3 | 레슨 | **보강 레슨**: 기존 레슨 일정 유지 + 추가 레슨 등록 (bookingType=SUPPLEMENTARY). 정규레슨 보강은 월간 회차 미포함 (별도), 쿠폰레슨 보강은 총 회차 포함 (1회 차감). 보강 사유 선택 입력 | High | Pending |
| FR-12b-4 | 수강생관리 | **코치 회원 검색**: 코치 스케줄 홈 우측 상단 🔍 버튼 → 회원 검색 화면. **검색 대상**: 코치와 연결된 수강생 (정규·쿠폰·보강·외부수강생 포함). **검색 방법**: 이름 또는 연락처 키워드 입력 → 실시간 필터링. **검색 결과 카드**: 프로필 이미지(이니셜) / 이름 / 레슨 유형(정규·쿠폰·외부) / 현재 회차 / 결제 상태 배지 / 다음 레슨 일정. **빈 상태**: 검색어 없을 시 전체 수강생 목록 표시, 검색 결과 없을 시 "검색 결과가 없어요" 안내. **카드 클릭**: 해당 수강생의 레슨 상세 풀팝업 진입. **진입점**: 코치 스케줄 홈 헤더 우측 🔍 아이콘 | High | Pending |
| ⛔ FR-13 | 코치 검증 | **(DEPRECATED — 마켓플레이스 신뢰성용. SaaS는 코치가 자기 학생 직접 등록·관리라 검증 배지 불필요)** **코치 검증 배지**: 자격증 업로드 → 관리자 심사 → 인증 배지 부여 | Medium | Pending |
...
| FR-14l | 관리자 | **제안 관리 (어드민)**: 모든 코치가 수강생에게 보낸 제안 통합 관리 화면. **유형**: 변경요청 제안(코치가 수강생 변경요청에 대해 제안한 가용 일정) + 보강 제안(결강된 레슨 보강 일정 제안). **4개 탭**: ① 전체 제안 ② 변경요청 제안 ③ 보강 제안 ④ 제안 이력. **상단 통계**: 총 제안/응답 대기/수락 완료/만료·거절. **필터**: 코치/수강생 검색, 상태(응답대기/수락/거절/만료/어드민강제), 기간, 코치별. **테이블**: 유형, 코치, 수강생, 원본 일정, 제안 일정(슬롯 칩, 3개+ 시 +N), 제안 시각, 상태, 홀드 만료(카운트다운), 관리자 조치(상세/강제 만료/재제안 요청). **상세 패널**: 코치/수강생 정보, 원본 일정, 제안 슬롯 목록, 홀드 정보, 알림톡 발송 이력, 어드민 직접 조치(강제 수락/거절/홀드 연장/해제/제안 취소/알림톡 재발송 + 사유 필수). **CSV 내보내기** 지원 | High | Pending |
| FR-14m | 코치/수강생/관리자 | **회차 통합/분할 (Session Merge & Split)**: 보강 처리 시 코치가 짧은 회차를 한 번에 길게 진행(통합)하거나 긴 회차를 짧게 여러 번 진행(분할)하도록 수강생에게 제안. **목적**: 주N회 짧은 레슨(예: 주2회 × 20분) 수강생의 보강 일정 매칭률 향상. **양방향 지원**: MERGE(N회 → 1회) / SPLIT(1회 → N회). **시간 합계 보존**: 결제 변동 없음(예: 20분 × 2 = 40분 × 1). **회차 카운트**: 통합 회차 1건이 sessionWeight=N으로 카운트되어 원 회차 수 보존. **코치 UI**: 7-0 보강 처리 바텀시트에 "회차 통합/분할 제안" 토글 + 라디오 + 대상 회차 선택. **수강생 UI**: 알림톡(MAKEUP_MERGE_PROPOSED/MAKEUP_SPLIT_PROPOSED) 수신 → 수락/거절. **어드민 정책 (9-9 결강 정책 하위)**: 통합/분할 활성/비활성, 수강생당 월 제안 한도(기본 1, 0=무제한), 같은 월 회차만 가능 토글, 한 그룹 최대 회차 수(기본 3). **DB 변경**: Booking에 sessionAdjustmentType/sessionWeight/mergeGroupId/mergedIntoBookingId 추가. **제약**: 1:1 개인만(1차), 미결제 회원 불가, LessonDuration enum 매칭 가능한 시간 합계만 허용 | High | Pending |
...
| 🆕 FR-15a | 학생 홈 | **(NEW — SaaS 피벗)** **학생 신규 홈 = 내 스케줄**: FR-03(마켓플레이스 홈) 대체. 학생 앱 진입 시 첫 화면이 코치 탐색이 아닌 **내 다가오는 레슨 캘린더**. 구성: ① 주간 미니 캘린더 (오늘 기준, 레슨 있는 날 표시) ② 오늘/이번 주 레슨 카드 (시간·코치명·상태) ③ 변경요청·보강 응답 알림 영역 (코치가 제안한 게 있으면 상단 노출) ④ 결제 안내 영역 (미결제 회차 있으면 코치한테 결제 완료 보고) ⑤ 하단 네비: 홈/변경요청/메시지/마이. 마켓플레이스 흔적(코치 검색·찜·신청 배너) 모두 제거 | High | Pending |
| 🆕 FR-15b | 수강생관리 | **(NEW — SaaS 피벗)** **코치 → 학생 초대**: FR-12b-2(외부 수강생 등록)의 강화·격상 버전. 코치가 학생을 전화번호로 초대 → SMS 또는 카카오 알림톡으로 가입 링크 발송 (`https://courtside.app/invite?token=...`) → 학생이 링크 진입 시 자동으로 그 코치에 매핑. **입력**: 학생 이름·전화번호(필수), 메모(선택). **API**: `POST /api/coaches/me/invitations { studentName, phone, memo }` → Invitation 토큰 생성 + 알림톡 발송. **Invitation 테이블**: token(UUID)·coachId·studentName·phone·status(PENDING/JOINED/EXPIRED)·createdAt·joinedAt·expiresAt(7일). 코치 화면(7-x)에 "초대한 학생" 목록 — 가입 대기 / 가입 완료 상태 분리 표시. 만료 시 재발송 가능 | High | Pending |
| 🆕 FR-15c | 인증·회원가입 | **(NEW — SaaS 피벗)** **학생 초대 가입 플로우**: 초대 링크 클릭 시 가입 단계 단축. 일반 가입(FR-01a~01d)에서 ① 역할 선택(FR-01b) **자동 학생 고정** ② 코치 둘러보기 안내(FR-01d "코치 둘러보기") **스킵** ③ 가입 완료 후 자동으로 초대한 코치와 매칭 + 첫 화면이 자기 코치 정보. 토큰 검증 + 만료/재사용 차단. UI: 초대 진입 페이지 (코치 프로필 미리보기 + "X 코치님이 회원님을 초대했어요" + 가입 진행 버튼) | High | Pending |
| 🆕 FR-16a | 학생 리텐션 | **(NEW — 리텐션 콘텐츠)** **오늘의 코트사이드 (Daily Card)**: 학생이 레슨 없는 날에도 매일 앱을 여는 데일리 훅. **매일 아침 8:00 카카오 알림톡 푸시 → 앱 진입 시 학생 홈 6-0 최상단 카드 + 풀스크린 상세(6-8)**. 5가지 모듈을 한 카드에 묶음: ① 🌤 **오늘 코트 컨디션** (학생 지역 기반 — 날씨/풍속/미세먼지 자동 판정, 🟢가능/🟡주의/🔴불가) ② 🇰🇷 **오늘 한국 선수 경기** (ATP/WTA 라이브 일정, 한국시간 자동 변환, 중계 채널) ③ 🏆 **어제의 프로 하이라이트** (1분 텍스트 요약 + 영상 링크, AI 자동 요약) ④ 📅 **이번 주 우리 지역 동호인 대회** (참가 가능한 것만 — 신청 마감일/레벨 매칭) ⑤ 🎯 **다음 레슨 D-N + 코치 미션** (FR-16a-mission 연동). **데이터 소스**: ① OpenWeatherMap API + 에어코리아 API ② ATP/WTA 공식 API + 자체 크롤링 ③ AI 요약 (Claude API) ④ KTA(대한테니스협회) 대회 일정 크롤링. **푸시 시각 설정**: 마이페이지에서 학생이 변경 가능 (기본 08:00, 옵션 06:00/07:00/08:00/09:00/끄기). **개인화**: 학생 등록 지역·NTRP·관심선수 기반 우선순위 정렬 | High | Pending |
| 🆕 FR-16b | 학생 리텐션 | **(NEW — 리텐션 콘텐츠)** **내 라켓 알람 (My Racket)**: 학생이 사용 라켓 등록 시 자동 알람·정보 제공. **등록 화면(6-9)**: 라켓 브랜드(윌슨/바볼랏/헤드/요넥스/프린스/던롭 등 마스터 데이터) → 모델 선택 → **마지막 스트링 교체일/텐션** 입력. **자동 알림 3종**: ① **같은 라켓 프로 선수 경기 결과** (예: "당신의 라켓을 쓰는 시너가 오늘 우승했어요" — `ProPlayer.racketModelId` 매칭) ② **신모델·할인 정보** (어드민이 `RacketNews` 테이블에 등록 → 학생 라켓과 매칭 시 푸시) ③ **스트링 교체 D-day** (마지막 교체일 + 학생 NTRP 기반 권장 주기 — 초급 3개월/중급 6주/상급 3주). **마이페이지 진입**: 6-2 마이페이지 → "내 라켓" 메뉴 → 6-9 화면. **데이터**: `Racket`(마스터) + `UserRacket`(학생 매핑) + `ProPlayer`(선수-라켓 매핑) + `RacketNews`(어드민 입력) | High | Pending |
| 🆕 FR-16c | 학생 리텐션 | **(NEW — 리텐션 콘텐츠)** **그랜드슬램 시청 가이드**: 4대 메이저(호주오픈 1월/롤랑가로스 5~6월/윔블던 6~7월/US오픈 8~9월) 기간 한정 특별 콘텐츠. **상시 화면(6-10)**: ① 다가오는/진행 중 그랜드슬램 카운트다운 카드 ② 한국 선수 출전 명단 + 라운드별 진행 상황 ③ 오늘/내일 주요 경기 (한국시간 자동 변환 — 호주오픈 +2h, 롤랑가로스/윔블던 -7h/-8h, US오픈 -13h) ④ 중계 채널 (SBS 스포츠, JTBC골프&스포츠, 쿠팡플레이 등) ⑤ AI 5분 하이라이트 요약 (경기 종료 30분 내 자동 생성) ⑥ 학생이 좋아하는 선수 즐겨찾기 (경기 시작 1시간 전 알림). **그랜드슬램 기간 외**: 다음 대회 카운트다운 + 지난 대회 회고. **진입점**: 마이페이지 → "그랜드슬램 시청 가이드" 메뉴 + 그랜드슬램 기간엔 6-0 학생 홈 상단 배너. **데이터 소스**: ATP/WTA 공식 일정 + 자체 큐레이션 (어드민 `Tournament` 테이블) | Medium | Pending |
