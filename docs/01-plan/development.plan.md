...
| 태스크 | DoD | 공수 |
|--------|-----|------|
| **SMS 본인인증 (학생·코치 공통)** | Solapi SMS 코드 발송 + 검증 위젯 + PhoneVerification 테이블 + 재발송/만료 정책. 학생·코치 프로필 등록 폼에 통합 (인증 완료 번호만 저장) | 1.5d |
...
**마일스톤**: 코치가 학생을 초대 → 학생이 가입 → 코치가 정기레슨 시간 설정 → 양쪽 홈에 표시. 코치 구독 결제 작동. **모든 가입자 전화번호는 SMS 인증 완료**.

### SMS 본인인증 정책 (Sprint 2 도입 예정)

- **대상**: 학생·코치 모두 프로필 등록 시 의무
- **벤더**: Solapi (알림톡과 동일 인프라)
- **인증 코드**: 6자리 숫자, 3분 유효, 최대 5회 재발송/시간(IP+번호 기준)
- **테이블**: `PhoneVerification` (id, phone, code, expiresAt, verifiedAt, attemptCount, ip)
- **저장 정책**: 인증 완료된 번호만 `users.phone` / `student_profiles.phone`에 저장
- **재인증 트리거**: 마이페이지에서 번호 변경 시 재인증 의무
...
| 알림톡: 학생 등록요청 (학생→코치) | 코치에게 신청 알림 — 현재 dev SMS로 임시 대체 중, Sprint 3에 알림톡 전환 | 0.5d |
| 알림톡: 등록요청 수락/거절 (코치→학생) | 코치 [수락]/[거절] 액션 시 학생에게 결과 알림 | 0.5d |
