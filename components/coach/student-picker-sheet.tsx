...
export function StudentPickerSheet({
  ...
  // 등록 성공 후 — 코치에게 보여줄 초대 메시지 (카톡/문자로 학생에 직접 전달)
  ...
  const handleRegister = () => {
    ...
    startRegTransition(async () => {
      ...
      // 등록 성공 — 초대 메시지 화면으로 전환, 부모에 reload 요청
      setRegSuccess({
        name,
  ...
  const handleCopyInvite = async () => {
    ...
    try {
      await navigator.clipboard.writeText(regSuccess.inviteMessage);
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    } catch (_) {
      // clipboard 권한 거부 — fallback: textarea select
      try {
        const ta = document.createElement("textarea");
        ta.value = regSuccess.inviteMessage;
                  ...
                  return (
                    <li key={slot.startMin}>
                      ...
                      >
                        ...
                        {blocked ? (
                          <span
                            className={`flex-none inline-flex items-center gap-0.5 text-[10px] font-semibold ${
                              isPast ? "text-ink-3" : "text-red-500"
                            }`}
                            aria-label={isPast ? "지난 시간" : "이미 잡힌 시간"}
                          >
                            {isPast ? (
                ...
                regSuccess ? (
                  <div className="px-2 pt-3 space-y-3">
                    <div className="rounded-xl bg-emerald-50 border border-emerald-200 px-3 py-3">
                      <div className="flex items-center gap-2">
                        <span className="w-6 h-6 rounded-full bg-emerald-500 text-white inline-flex items-center justify-center flex-none">
                          <svg viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5" aria-hidden>
                            <path fillRule="evenodd" d="M16.7 5.3a1 1 0 010 1.4l-7 7a1 1 0 01-1.4 0l-3-3a1 1 0 111.4-1.4l2.3 2.3 6.3-6.3a1 1 0 011.4 0z" clipRule="evenodd" />
                          </svg>
                        </span>
                        <div className="text-sm font-extrabold text-emerald-700">
                          {regSuccess.name}님 등록 완료
                        </div>
                      </div>
                      <p className="mt-1.5 text-[11px] text-emerald-700 leading-relaxed">
                        {regSuccess.isPlaceholder
                          ? "아직 미가입 상태예요. 아래 초대 메시지를 카톡·문자로 전달하면, 같은 전화번호로 가입 시 자동 연결돼요."
                          : "이미 가입된 회원이라 바로 매칭되었어요. 안내 메시지를 보내드릴 수 있어요."}
                      </p>
                    </div>

                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <label className="block text-[11px] font-semibold text-ink-2">초대 메시지</label>
                        {copied && (
                          <span className="text-[10px] font-bold text-emerald-600">복사됨!</span>
                        )}
                      </div>
                      <textarea
                        readOnly
                        value={regSuccess.inviteMessage}
                        onClick={(e) => (e.currentTarget as HTMLTextAreaElement).select()}
                        rows={7}
                        className="w-full rounded-xl border border-line bg-soft px-3.5 py-2.5 text-[12px] text-ink leading-relaxed outline-none focus:ring-2 focus:ring-primary/40 resize-none tabular-nums"
                      />
                    </div>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 h-11 rounded-xl border border-line bg-surface text-xs font-bold text-ink-2 hover:bg-soft transition active:scale-[0.99] inline-flex items-center justify-center gap-1.5"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 h-11 rounded-xl bg-primary text-white text-xs font-bold hover:opacity-90 transition active:scale-[0.99] inline-flex items-center justify-center gap-1.5"
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                    ...
                    <button
                      ...
                      className="w-full h-11 rounded-xl bg-soft text-xs font-semibold text-ink-2 hover:bg-line transition"
                    >
                      목록으로 돌아가기
                    </button>
                  </div>
                  ...
                  <div className="px-2 pt-3 space-y-3">
                    ...
                    >
                      ...
                      {regPending ? "등록 중…" : "등록하고 초대 메시지 받기"}
                    </button>
                  </div>
                )
              ) : filtered.length === 0 ? (
